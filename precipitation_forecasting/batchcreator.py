...
class DataGenerator(keras.utils.Sequence):
    ...
    def prep_data(self, X, y):
        ...
        if self.crop_y:
            y = self.crop_center(y, cropx=440, cropy=440)

        if self.downscale256:
            # First make the images square size

            X = self.crop_center(X, cropx=440, cropy=440)
            ...
            for i in range(N):
                for j in range(N):
                    X[:][calibration * i:calibration * (i + 1)][calibration * j:calibration * (j + 1)] = gpd(X[:][calibration * i:calibration * (i + 1)][calibration * j:calibration * (j + 1)], u=gpd_u[i][j], delta=gpd_delta[i][j], e=gpd_e[i][j])
                    y[:][calibration * i:calibration * (i + 1)][calibration * j:calibration * (j + 1)] = gpd(y[:][calibration * i:calibration * (i + 1)][calibration * j:calibration * (j + 1)], u=gpd_u[i][j], delta=gpd_delta[i][j], e=gpd_e[i][j])
    ...
    def load_x(self, x_ID):
        if self.load_prep:
            ...
            dt = datetime.strptime(x_ID[0:12], '%Y%m%d%H%M')
    ...
    def load_y(self, y_ID):
        if self.y_is_rtcor:
            ...
            year = y_ID[:4]
            month = y_ID[4:6]
            path = self.y_path + year + '/' + month + '/' + conf.prefix_aart + y_ID +'_NL.h5'
            y = self.load_h5(path)
        return y
...
def undo_prep(x, norm_method=None, r_to_dbz=False, downscale256=True, resize_method = tf.image.ResizeMethod.BILINEAR):
    if norm_method:
    ...
    if downscale256:
        ...
        x =  tf.convert_to_tensor([tf.image.resize(img, (440, 440), method=resize_method) for img in x])
        ...
        for i in range(N):
            for j in range(N):
                x[:][calibration * i:calibration * (i + 1)][calibration * j:calibration * (j + 1)] = inv_gpd(x[:][calibration * i:calibration * (i + 1)][calibration * j:calibration * (j + 1)], u=gpd_u[i][j], delta=gpd_delta[i][j], e=gpd_e[i][j])

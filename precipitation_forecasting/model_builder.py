...
from TrajGRU import SaConvLSTM
...
def convRNN_block(x, filters, kernel_size, strides, rnn_type='GRU', padding='same', return_sequences=True, 
    ...
    #prevent nan tensor value
    #x = tf.clip_by_norm(x, 5)
...
def conv_block(x, filters, kernel_size, strides, padding='same', name=None, relu_alpha=0.2, 
    ...
    if batch_norm:
        x = tf.keras.layers.BatchNormalization()(x)    
    if output_layer:
        x = tf.keras.activations.relu(x, max_value=9.8)
...
def encoder(x, rnn_type, relu_alpha):
    # Downsample 1a
    x = tf.keras.layers.Conv3D(filters=8, kernel_size=(1, 3, 3), strides=(1, 2, 2), padding='same',
                               name='Downsample1a')(x)
    x = tf.keras.layers.LeakyReLU(relu_alpha)(x)
...
def decoder(x, rnn_type, relu_alpha):
    ...
    # Upsample
    x = tf.keras.layers.Conv3DTranspose(name='Upsample1', filters=192, kernel_size=(1, 4, 4), strides=(1, 2, 2),
                                        padding='same')(x)
    ...
    # Upsample
    x = tf.keras.layers.Conv3DTranspose(name='Upsample2', filters=192, kernel_size=(1, 5, 5), strides=(1, 3, 3),
                                        padding='same')(x)
    ...
    x = tf.keras.layers.Conv2DTranspose(name='Upsample3', filters=8, kernel_size=(5, 5), strides=(2, 2),
                                        padding='same')(x)
    x = tf.keras.layers.LeakyReLU(relu_alpha)(x)
    x = tf.keras.layers.Conv2DTranspose(name='Conv', filters=1, kernel_size=1, strides=1, padding='same',
                                        activation='tanh')(x)
...
class GAN(tf.keras.Model):
    ...
    def loss_rec(self, target, pred, MAE = True):
        ...
        target = target.numpy()
        pred = pred.numpy()
        target = inv_unit_frechet(target,u=u,delta=delta)
        pred = inv_unit_frechet(pred, u=u, delta=delta)
        '''
        g_loss_mse = self.loss_mse(target, pred)

        #g_loss_mse = balanced_mse(target, pred)
    ...
    def train_disc_seq(self, inp, labels, train = True ):
        if train:
            with tf.GradientTape() as tape:
                ...
                #prevent loss to become 0
                #d_loss_seq = tf.clip_by_value(d_loss_seq, 1e-8, 10)
            grads = tape.gradient(d_loss_seq, self.discriminator_seq.trainable_weights)
            #prevent tidubaozha
            #grads = tf.clip_by_value(grads, clip_value_min = -10, clip_value_max = 10)
            #grads = tf.clip_by_norm(grads, 100)
            '''
            for gradsitem in grads:
                gradsitem = tf.clip_by_norm(gradsitem, 5)
            '''
        ...
        if train:
            with tf.GradientTape() as tape:
                ...
                for i in range(self.y_length):
                    ...
                    # prevent loss to become 0
                    #d_loss_frame = tf.clip_by_value(d_loss_frame, 1e-8, 10)
        ...
        if train:
            for _ in range(self.g_cycles):
                with tf.GradientTape() as tape:
                    ...
                    g_loss_adv = adv_loss_frame + 0.5 * adv_loss_seq
                    # prevent loss to become 0
                    #g_loss_adv = tf.clip_by_value(g_loss_adv, 1e-8, 10)
                    g_loss_rec = self.loss_rec(ys, generated_images, self.rec_with_mae)
                    # prevent loss to become 0
                    #g_loss_rec = tf.clip_by_value(g_loss_rec, 1e-8, 10)
                    g_loss =  self.l_adv * g_loss_adv  + self.l_rec * g_loss_rec
                    #print("g_loss")
                    #print(g_loss)
                grads = tape.gradient(g_loss, self.generator.trainable_weights)
                # prevent tidubaozha
                #grads = tf.clip_by_value(grads, -10, 10)
                #grads = tf.clip_by_norm(grads, 100)
                '''
                for gradsitem in grads:
                    gradsitem = tf.clip_by_norm(gradsitem, 5)
    ...
    def model_step(self, batch, train = True):
        ...
        #zhiyiarr = tf.make_ndarray(generated_images)
        zhiyiarr = generated_images.numpy()
        flag_zhiyi = False
        for i in range(256):
            for j in range(256):
                if(zhiyiarr[i][j] > 1):
        ...
        plt.imshow(zhiyiarr[0])
        plt.colorbar()
        plt.show()
        
        zhiyitarget = ys.numpy()
        plt.imshow(zhiyitarget[0][0])
        plt.colorbar()
        plt.show()
        '''
